...
class MySceneCfg(InteractiveSceneCfg):
    ...
    ray_caster_camera = RayCasterCameraCfg(
        ...
        ray_alignment='yaw',
        pattern_cfg=patterns.PinholeCameraPatternCfg(width=10, height=10, focal_length=11.41,
                                                     horizontal_aperture=20.955, vertical_aperture=12.64),
        debug_vis=False,
        max_distance=10.0,  # 这个给了也不会裁减
        mesh_prim_paths=["/World/ground"],
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformThresholdVelocityHighCommandCfg(
        asset_name="robot",
        ...
        debug_vis=True,
        ranges=mdp.UniformThresholdVelocityHighCommandCfg.Ranges(
...
class LocomotionVelocityRoughEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        if self.scene.ray_caster_camera is not None:
            self.scene.ray_caster_camera.update_period = (self.decimation * self.sim.dt) * 2
