...
parser.add_argument("--input_file", type=str, required=True, help="The path to the input motion mjcf file.")
...
parser.add_argument("--use_qvel", type=bool, default=False, help="true/false to use qvel.")
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        self.model = mujoco.MjModel.from_xml_path(self.motion_file)
        motion = torch.from_numpy(self.model.key_qpos)
        ...
        if self.use_qvel:
            motion_qvel = torch.from_numpy(self.model.key_qvel)
            motion_qvel = motion_qvel.to(torch.float32).to(self.device)
            self.motion_base_lin_vels_input = motion_qvel[:, :3]
            self.motion_base_ang_vels_input = motion_qvel[:, 3:6]
            self.motion_dof_vels_input = motion_qvel[:, 6:]
    ...
    def _interpolate_motion(self):
        ...
        if self.use_qvel:
            self.motion_base_lin_vels = self._lerp(
                self.motion_base_lin_vels_input[index_0],
                self.motion_base_lin_vels_input[index_1],
                blend.unsqueeze(1),
            )
            self.motion_base_ang_vels = self._lerp(
                self.motion_base_ang_vels_input[index_0],
                self.motion_base_ang_vels_input[index_1],
                blend.unsqueeze(1),
            )
            self.motion_dof_vels = self._lerp(
                self.motion_dof_vels_input[index_0],
                self.motion_dof_vels_input[index_1],
                blend.unsqueeze(1),
            )
        print(
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    motion = MotionLoader(
        ...
        use_qvel=args_cli.use_qvel
    )

    # Extract scene entities
    robot = scene["robot"]
    robot_joint_indexes = robot.find_joints(motion.joint_names, preserve_order=True)[0]
