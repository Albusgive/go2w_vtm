...
class JumpCurveDynamicCalculatorW:
    ''' 在body_z-terrain_z为z原点按照 上面世界坐标系的轨迹计算跳跃轨迹'''
    def __init__(
        self,
        max_envs: int,
        ...
        a2: float = -1.0,
        g: float = -9.8,
        default_start_height: float = 0.4,
        ...
        Parameters:
            ...
            default_end_height: 跳跃结束高度 (m)
        """
        self.device = device
        self.max_envs = max_envs
        self.is_track_gravity = is_track_gravity
        ...
        self.h0 = torch.full((max_envs,), default_start_height, 
                                   device=device)
        self.a1 = 2 * (self.h0 - self.h1) / (self.t1 ** 3)
        self.b1 = 3 * (self.h1 - self.h0) / (self.t1 ** 2)
        ...
        self.end_heights = torch.full((max_envs,), default_end_height, 
                                    device=device)
        self.stile = torch.zeros((max_envs,), device=device)

    def set_parameters(self, env_mask: torch.Tensor, a_basemax: torch.Tensor,
                       start_heights: torch.Tensor = None, end_heights: torch.Tensor = None) -> None:
        """
        ...
        start_heights = start_heights.to(self.device)
        
        # Update stored parameters only for selected environments
        self.a_basemax[env_mask] = a_basemax[env_mask].to(torch.float32)
        self.h0[env_mask] = start_heights[env_mask].to(torch.float32)
        
        self.a1[env_mask] = 2 * (self.h0[env_mask] - self.h1) / (self.t1 ** 3)
        self.b1[env_mask] = 3 * (self.h1 - self.h0[env_mask]) / (self.t1 ** 2)
    ...
    def _compute_segment(self, env_mask: torch.Tensor, x: torch.Tensor, mode: str) -> torch.Tensor:
        ...
        self.stile[:] = 0.0
        self.stile[mask1] = 1.0
        self.stile[mask2] = 2.0
        
        
        # Segment 1: 0 ≤ x ≤ t1
        if mode == "height":
            result = torch.where(mask1, self.a1 * x**3 + self.b1 * x**2 + self.h0, result)
        elif mode == "velocity":
        ...
        if self.is_track_gravity:
            self.stile[mask3] = 3.0
    ...
    @property
    def get_stile(self) -> torch.Tensor:
        return self.stile
